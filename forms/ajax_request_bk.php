...
else if($action=="view_tray_stocks")
{
	...
	$sel_qry = "SELECT COUNT(*) as allcount FROM( SELECT name, category, date, (SUM(outward) - SUM(inward)) as balance FROM `tray_transactions` where category ='Customer' ";
	...
	$data_sql = "SELECT name, category, date ,(SUM(outward) - SUM(inward)) as balance FROM `tray_transactions` WHERE category='Customer'";
     if($from!="" && $to!="")
    {
...
$sel_qry = "SELECT COUNT(*) as allcount FROM( SELECT name, category, date, (SUM(outward) - SUM(inward)) as balance FROM `tray_transactions` where category ='Supplier' ";
	 if($from!="" && $to!="")
    {
       $sel_qry .=" AND (date >='$from' AND date<='$to')"; 
     //array_push($filter, " (date >='$from' AND date<='$to')");  
	...
	$data_sql = "SELECT name, category, date ,(SUM(outward) - SUM(inward)) as balance FROM `tray_transactions` WHERE category='Supplier'";
     if($from!="" && $to!="")
    {
     $data_sql .=" AND (date >='$from' AND date<='$to')";   
     //array_push($filter, " (date >='$from' AND date<='$to')");  
...
else if($action=="view_tray_inventory")
{
	...
	$sel_qry = "SELECT count(*) as allcount FROM `tray_transactions` WHERE category='Customer'";
    ...
    $data_sql = " SELECT * FROM `tray_transactions` WHERE category='Customer' ";
	...
	$data_sql .="group by name,date order by id desc";
...
else if($action=="view_tray_inventory_supplier")
{
	...
	$sel_qry = "SELECT count(*) as allcount FROM `tray_transactions`  ";
		 $counter = 0;
...
//$data_sql .="GROUP BY name";
    ...
    $data_sql = "SELECT * FROM `tray_transactions` WHERE category='Supplier' ";
...
else if($action=="view_tray_inventory_trays")
{
    ...
    $data_sql = "SELECT * FROM `tray_transactions` WHERE category='Trays'";
...
else if($action=="view_tray_inventory_all")
{
    ...
    $data_sql = " SELECT id,name,category FROM `tray_transactions` ";
	...
	while ($data_row = $data_qry->fetch(PDO::FETCH_ASSOC)) {
    	...
    	$select_qry4= "SELECT sum(outward) - sum(inward) as farmer_sum FROM `tray_transactions` WHERE name='".$data_row["name"]."' AND category='Supplier' group by name";
	    $select_sql4=$connect->prepare($select_qry4);
    	$select_sql4->execute();
    	$select_row4 = $select_sql4->fetch(PDO::FETCH_ASSOC);
    	...
    	$select_qry6= "SELECT sum(outward) - sum(inward) as customer_sum FROM `tray_transactions` WHERE name='".$data_row["name"]."' AND category='Customer' group by name";
	    $select_sql6=$connect->prepare($select_qry6);
    	$select_sql6->execute();
    	$select_row6 = $select_sql6->fetch(PDO::FETCH_ASSOC);
...
else if($action=="view_cash_carry_all")
{
    ...
    $data_sql = "SELECT cash_no as id,customer_name as name,date as Date,total_bill_amount as Bill_Amount, payment, cash_no as customer_id,credit_type as credit_type FROM sar_cash_carry WHERE cash_no LIKE '%CC%' GROUP BY cash_no UNION SELECT sales_no,invoice.customer_name,invoice.date,invoice.total_bill_amount as bill_amount, sum(pay.amount) as payment, invoice.sales_no, invoice.credit_type FROM sar_sales_invoice as invoice, sar_sales_payment as pay Where invoice.sales_no = pay.customer_id and is_revoked is NULL GROUP By sales_no";

    if ($filter_str != ""){
        $data_sql = "SELECT cash_no as id,customer_name as name,date as Date,total_bill_amount as Bill_Amount, payment, cash_no as customer_id,credit_type as credit_type FROM sar_cash_carry as invoice WHERE ".$filter_str. " AND cash_no LIKE '%CC%' GROUP BY cash_no UNION SELECT sales_no,invoice.customer_name,invoice.date,invoice.total_bill_amount as bill_amount, sum(pay.amount) as payment,invoice.sales_no, invoice.credit_type FROM sar_sales_invoice as invoice, sar_sales_payment as pay Where " .$filter_str. " AND invoice.sales_no = pay.customer_id and is_revoked is NULL GROUP By sales_no";
    }
	...
	$data = array();
	while ($data_row = $data_qry->fetch(PDO::FETCH_ASSOC)) {
	   $data_row["waiver_discount"] = $sales_no_discount[$data_row["customer_id"]];
