...
$purchaseqry="SELECT * FROM sar_revenue where id='$purid'";
...
<option value="Other Expense" <?= ($purchaserow['particulars']=="Interest")?"selected=selected":"" ?>>Interest</option>  
<option value="Other Expense" <?= ($purchaserow['particulars']=="Gift")?"selected=selected":"" ?>>Gift</option>  
...
//   $exp_no = "EXP_".date("Ym")."0".$Last_id;
  ...
  if($type=="Supplier" || $type=="Customer"){
    $balance_qry="SELECT * FROM sar_revenue where grp='$grp' and purchased_from='$purchased_from' and id!='$purid' ORDER BY id DESC LIMIT 1 ";
