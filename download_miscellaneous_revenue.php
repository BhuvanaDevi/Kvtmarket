...
$objPHPExcel ->getActiveSheet()->setTitle("Get Miscellaneous Revenue");
...
$objPHPExcel->getActiveSheet()->setCellValue('C1','Revenue No');
$objPHPExcel->getActiveSheet()->setCellValue('D1','Particulars');
$objPHPExcel->getActiveSheet()->setCellValue('E1','Amount');
$objPHPExcel->getActiveSheet()->setCellValue('F1','Mode');
$objPHPExcel->getActiveSheet()->setCellValue('G1','Remarks');
$objPHPExcel->getActiveSheet()->setCellValue('H1','Updated By');


$sql="SELECT * FROM `sar_miscellaneous_revenue` WHERE (date>='".$from."' AND date<='".$to."')";
...
if(mysqli_num_rows($res)>0)
{
    ...
    while($approval_row = mysqli_fetch_object($res)) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('C'.($i), $approval_row->revenue_no);
        $objPHPExcel->getActiveSheet()->setCellValue('D'.($i), $approval_row->particulars);
        $objPHPExcel->getActiveSheet()->setCellValue('E'.($i), $approval_row->amount);
        $objPHPExcel->getActiveSheet()->setCellValue('F'.($i), $approval_row->mode);
        $objPHPExcel->getActiveSheet()->setCellValue('G'.($i), $approval_row->remarks);
        $objPHPExcel->getActiveSheet()->setCellValue('H'.($i), $approval_row->updated_by);
        $i++;
    }    
}
...
//exit;
$filename="Get Miscellaneous Revenue";
