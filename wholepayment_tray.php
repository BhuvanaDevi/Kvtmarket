...
// $tray=$_REQUEST["tray"];
if($from!="" && $to!="" && $type==""){
$data_sql="SELECT * FROM `trays` where category='Customer' and (date >='$from' AND date<='$to')";
// group by name
$data_qry= $connect->prepare($data_sql);
$data_qry->execute();
}
else{
  $data_sql="SELECT * FROM `trays` where category='Customer' and type='$type' and (date >='$from' AND date<='$to')";
// group by name
$data_qry= $connect->prepare($data_sql);
$data_qry->execute();
}
// print_r($data_qry);die();
//$data_row = $data_qry->fetch(PDO::FETCH_ASSOC);
//print_r($data_row['name']);die();	   
...
// $sel_qry2 = "SELECT * FROM `trays` WHERE category='Customer' AND (date >='$from' AND date<='$to') AND name='$customer' AND type='$tray'";
// $data_qry2= $connect->prepare($sel_qry2);
// $data_qry2->execute();

// $sel_qryn = "SELECT * FROM `trays` WHERE category='Customer' AND (date >='$from' AND date<='$to') AND name='$customer' AND type='$tray' order by id desc limit 1";
...
$row;
        ...
        // $pdf->Ln(44);
        $pdf->SetFont('Arial','B',18);
        $pdf->Cell(180,10,'Tray Wise - Customer',0,0,'C');
        $pdf->Ln(10);
if($type=="Small Tray")
{        $pdf->SetFont('Arial','B',18);
        $pdf->Cell(180,10,'Tray Type - Small Tray',0,0,'C');
        $pdf->Ln(10);
}
if($type=="Big Tray"){
  $pdf->SetFont('Arial','B',18);
  $pdf->Cell(180,10,'Tray Type - Big Tray',0,0,'C');
  $pdf->Ln(10);
}
...
$pdf->Ln(10);
    ...
    //     $pdf->SetFont('Arial','',14);
    //     // $pdf->SetTopMargin(50);
    //     $pdf->Cell(10,10,$resultn['date'],0,0,false);
    //     $pdf->Ln();
    //     $pdf->SetFont('Arial','B',14);
    ...
    //   $pdf->Ln(20);
      ...
      $pdf->Cell(20,10,'Inward',0,0,'R',true);
      $pdf->Cell(20,10,'Outward',0,0,'R',true);
      $pdf->Cell(20,10,'Inhand',0,0,'R',true);
      $pdf->Cell(20,10,'Small Tray',0,0,'R',true);
      $pdf->Cell(20,10,'Big Tray',0,0,'R',true);
      $pdf->Ln();
        
      
    while($result = $data_qry->fetch(PDO::FETCH_ASSOC))
     {
      $pdf->SetTextColor(1, 0, 4);
        $pdf->SetFont('Arial','',10);
      $cusid=$result['name'];

      $sqll="select * from sar_customer where customer_no='$cusid'";
      $data_qryl= $connect->prepare($sqll);
$data_qryl->execute();
$data_qryv = $data_qryl->fetch(PDO::FETCH_ASSOC);
$grp="select * from sar_sales_invoice where customer_id='$cusid'";
$grop= $connect->prepare($grp);
$grop->execute();
$grp = $grop->fetch(PDO::FETCH_ASSOC);

$pdf->Cell(20,10,$grp['groupname'],0, 0, "R");
$pdf->Cell(20,10,$data_qryv['customer_name'],0, 0, "R");
        $pdf->Cell(25,10,$result['date'],0,0,'R');
        // $pdf->Cell(25,10,$result['type'],0,0,'R');
        $pdf->Cell(20,10,$result['type'],0,0,'R');
        $pdf->Cell(15,10,$result['inward'],0,0,'R');
        $pdf->Cell(20,10,$result['outward'],0,0,'R');
...
// //$pdf->Cell(87 ,6,'','T',0);
// // $pdf->Cell(45 ,10,'Sub Total',1,0,'C');
// $pdf->Cell(47, 10, 'Total Trays', "T", 0, "C");
// //$pdf->Cell(47,10,$row->total_bill_amount,1,0,'C');
// $pdf->Cell(40, 10, $resultn['inhand'], 'T', 0, "R");
// // $pdf->Cell(45 ,10,'Sub Total',1,0,'C');
// $pdf->Cell(47, 10, 'Total Trays', "T", 0, "C");
// //$pdf->Cell(47,10,$row->total_bill_amount,1,0,'C');
// $pdf->Cell(40, 10, ($resultn['inhand']*100), 'T', 0, "R");
//$pdf->Ln(10);
// $pdf->Cell(87 ,6,'',0,0);
