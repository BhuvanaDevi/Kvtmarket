...
$sel_qry2 = "SELECT * FROM `sar_patti_payment` WHERE supplier_id='$supplier' order by id desc limit 1";
$data_qry2= $connect->prepare($sel_qry2);
$data_qry2->execute();
$resultb=$data_qry2->fetch(PDO::FETCH_OBJ);
//print_r($resultb->date);die();

$sel_qryn = "SELECT * FROM `sar_patti_payment` WHERE supplier_id='$supplier' AND (payment_date >='$from' AND payment_date<='$to')";
...
$sel_qryt = "SELECT * FROM `trays` WHERE name='$supplier' and type='Small Tray' order by id desc limit 1";
$data_qryt= $connect->prepare($sel_qryt);
$data_qryt->execute();
$resultt = $data_qryt->fetch(PDO::FETCH_ASSOC);
$small_tray=$resultt['inhand']!=0?$resultt['inhand']:0;

$sel_qryb = "SELECT * FROM `trays` WHERE name='$supplier' and type='Big Tray' order by id desc limit 1";
...
$grop->execute();
$group=$grop->fetch(PDO::FETCH_OBJ);
...
$row;
       ...
       $pdf->SetFont('Arial','',14);
        ...
        $pdf->SetFont('Arial','',14);
        // $pdf->SetTopMargin(50);
        $pdf->Cell(10,10,$group->groupname,0,0,false);
        $pdf->Ln();
        $pdf->SetFont('Arial','B',14);
      ...
      $pdf->Cell(25,10,'Given',0,0,'R',true);
      $pdf->Cell(25,10,'Balance',0,0,'R',true);
      $pdf->Cell(25,10,'Discount',0,0,'R',true);
      $pdf->Cell(25,10,'Small Tray',0,0,'R',true);
      $pdf->Cell(25,10,'Big Tray',0,0,'R',true);
    ...
    while($result = $data_qryn->fetch(PDO::FETCH_ASSOC))
     {
      $pdf->SetTextColor(1, 0, 4);
        ...
        $sqlp = "SELECT * FROM  sar_patti WHERE patti_id='$patid'";
 ...
 //   print_r($resultsp);die();
      ...
      $pdf->Cell(20,10,$result['payment_date'],0,0,'R');
      $pdf->Cell(30,10,$resultsp->total_bill_amount,0,0,'R');
      $pdf->Cell(25,10,$result['amount'],0,0,'R');
      $pdf->Cell(25,10,$result['balance'],0,0,'R');
      $pdf->Cell(25,10,$result['discount'],0,0,'R');
      $pdf->Cell(25,10,$small_tray,0,0,'R');
      $pdf->Cell(25,10,$big_tray,0,0,'R');
