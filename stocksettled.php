...
// if($customer==""){
$sel_qryn = "SELECT * FROM `sar_stock` WHERE payment_status=1 and (date >='$from' AND date<='$to') and return_status=0";
...
$pdf->Cell(180,10,'Stock Settled',0,0,'C');
$pdf->Ln(10);
$pdf->SetFont('Arial','B',12);
   ...
   while($result = $data_qryn->fetch(PDO::FETCH_ASSOC))
     {
        ...
        $sel_qryn = "SELECT * FROM `sar_stock` WHERE purchase_id='$result[purchase_id]'";
$data_qryn= $connect->prepare($sel_qryn);
$data_qryn->execute();
$results = $data_qryn->fetch(PDO::FETCH_ASSOC);

    $pdf->Cell(20,10,$results['group_name'],0,0,'L');
         $pdf->Cell(25,10,$results['supplier_name'],0,0,'R');
       $pdf->Cell(30,10,$result['date'],0,0,'R');
       $pdf->Cell(30,10,$results['quality_name'],0,0,'R');
       $pdf->Cell(15,10,$results['quantity'],0,0,'R');
       $pdf->Cell(15,10,$results['rate'],0,0,'R');
       $pdf->Cell(30,10,$results['stock_amount'],0,0,'R');
