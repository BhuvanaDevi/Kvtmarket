...
$sqls = "SELECT * FROM  sar_customer WHERE customer_no='$customer_id'";
...
$row;
        ...
        $pdf->Cell(180,10,'Sales Invoice Bill',0,0,'C');
        $pdf->Ln(10);

     $pdf->SetFont('Arial','B',14);
        $pdf->Cell(45 ,10,'Invoice Id',0,0,'L',false);
        $pdf->SetFont('Arial','',14);
        $pdf->Cell(47 ,10,$customer_id,0,0,'L',false);
        // $pdf->Ln();
            ...
            $pdf->SetFont('Arial','',14);
            $pdf->Cell(45 ,10,$sup->customer_name,0,0,'L',false);
           
                    $pdf->Cell(50,10,'Group Name',0,0,'R',false);
            $pdf->SetFont('Arial','',14);
            $pdf->Cell(25,10,$sup->contact_number1,0,0,'L',false);
      ...
      $pdf->Ln();
        ...
        $pdf->SetFont('Arial','',12);
            ...
            $old="select * from payment_sale where customerid = '$customer_id' and date<'$date' order by id desc limit 1";
...
//  print_r($old1);die();
    ...
    $today1="select SUM(dis) as dis from payment_sale where customerid = '$customer_id' and date='$date' and (paymentmode='cash' or paymentmode='percentage')";
...
$today2="select SUM(pay) as pay from payment_sale where customerid = '$customer_id' and date='$date' and saleid='OB'";
