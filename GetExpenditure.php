...
if($category!="" && $from!="" && $to!=""){
    $sel_qry2 = "SELECT * FROM `sar_expenditure` WHERE (date >='$from' AND date<='$to') AND revenue='$category'";
//print_r($sel_qry2);die();
$data_qry2= $connect->prepare($sel_qry2);
$data_qry2->execute();
}
...
$pdf->Ln(15);
    ...
    //  $pdf->Ln(20);
      ...
      $pdf->SetFont('Arial','B',16);
        $pdf->Cell(25,10,'Exp No',0,0,'L',true);
        $pdf->Cell(30,10,'Date',0,0,'R',true);
        $pdf->Cell(65,10,'Category',0,0,'R',true);
    ...
    while($result = $data_qry2->fetch(PDO::FETCH_ASSOC))
     {
      $pdf->SetTextColor(1, 0, 4);
        ...
        $pdf->Cell(31,10,$result['date'],0,0,'R');
            $pdf->Cell(55,10,$result['revenue'],0,0,'R');
            $pdf->Cell(25,10,$result['payment_mode'],0,0,'R');
