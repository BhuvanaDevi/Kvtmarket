...
$objPHPExcel->getActiveSheet()->setTitle("Get Active Customer Report");
$sql="SELECT * FROM `sar_customer` WHERE is_active=1";
...
if(mysqli_num_rows($res)>0)
{
    ...
    while($approval_row = mysqli_fetch_object($res)) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('B'.($i), $approval_row->customer_no);
        $objPHPExcel->getActiveSheet()->setCellValue('C'.($i), $approval_row->customer_name);
...
//Rename the worksheet
$objPHPExcel ->getActiveSheet()->setTitle("Get Inactive Customer Report");
$sql="SELECT * FROM `sar_customer` WHERE is_active=0";
