...
<script>
   ...
   if(invoice=="Patti"){
    ...
    if(url == "http://localhost:8080/ab_live/pays.php?invoice=<?=$_GET['invoice']?>&supplier=<?=$_GET['supplier']?>&customer=<?=$_GET['customer']?>&submit=submit"){
window.location.replace("http://localhost:8080/ab_live/pays.php?invoice=Patti&supplier=<?=$_GET['supplier']?>&submit=submit");
    }
      $("#supp").show();
    $("#custom").hide();
    ...
    var url = window.location.href;
    if(url == "http://localhost:8080/ab_live/pays.php?invoice=<?=$_GET['invoice']?>&supplier=<?=$_GET['supplier']?>&customer=<?=$_GET['customer']?>&submit=submit"){
window.location.replace("http://localhost:8080/ab_live/pays.php?invoice=Sales&customer=<?=$_GET['customer']?>&submit=submit");
    }
    $("#custom").show();
    $("#supp").hide();
   }
...
if(isset($_POST['remainings'])){
    ...
    // print_r($_POST);die();
if($patid!=""){    
    $sql="select * from sar_patti where supplier_id='$supplierid' and is_active!=0 order by total_bill_amount asc";
    $exe=mysqli_query($con,$sql);
    $no=mysqli_num_rows($exe);
    ...
    while($row=mysqli_fetch_assoc($exe)){
        ...
        if($row['remain']==0){
            ...
            //   print_r($upsbal."a".$amt.$totpati);die();
              $exesbal=mysqli_query($con,$upsbal);

              
    $ins="INSERT INTO `sar_patti_payment`( `payment_id`, `payment_date`, `amount`, `payment_mode`, `balance`, `is_revoked`, `discount`, `discount_type`, `supplier_id`,`patti_id`) VALUES('$pay_id','$pay_date',$amt,'$payment_mode',$ba,$is_revoked,$dis,'$amttype','$row[supplier_id]','$row[patti_id]')";           
    ...
    $res2=mysqli_query($con,$fin_trans_qry);
            ...
            //   $totpati=$row['total_bill_amount'];
              ...
              header("location:pays.php?invoice=Patti&supplier=$supplier");
                ...
                $ba = $totpati-$rems-$amt;
                $upsbal="update sar_patti set remain='$ba' where patti_id='$row[patti_id]'";
                //   print_r($upsbal."a1".$amt.$totpati);die();
                  $exesbal=mysqli_query($con,$upsbal);
        ...
        $res2=mysqli_query($con,$fin_trans_qry);
            ...
            $ins="INSERT INTO `sar_patti_payment`( `payment_id`, `payment_date`, `amount`, `payment_mode`, `balance`, `is_revoked`, `discount`, `discount_type`, `supplier_id`,`patti_id`) VALUES('$pay_id','$pay_date',$amt,'$payment_mode',$bal2,$is_revoked,$dis,'$amttype','$row[supplier_id]','$row[patti_id]')";           
    $exeins=mysqli_query($con,$ins);           
    ...
    $res2=mysqli_query($con,$fin_trans_qry);
            ...
            $ins="INSERT INTO `sar_patti_payment`( `payment_id`, `payment_date`, `amount`, `payment_mode`, `balance`, `is_revoked`, `discount`, `discount_type`, `supplier_id`,`patti_id`) VALUES('$pay_id','$pay_date',$amt,'$payment_mode',$bal3,$is_revoked,$dis,'$amttype','$row[supplier_id]','$row[patti_id]')";           
    $exeins=mysqli_query($con,$ins);           
    ...
    break;
        ...
        if($val<0) {
            ...
            //   print_r($upsbal."h");die();
              ...
              $ins="INSERT INTO `sar_patti_payment`( `payment_id`, `payment_date`, `amount`, `payment_mode`, `balance`, `is_revoked`, `discount`, `discount_type`, `supplier_id`,`patti_id`) VALUES('$pay_id','$pay_date',$amt,'$payment_mode',$val,$is_revoked,$dis,'$amttype','$row[supplier_id]','$row[patti_id]')";           
    $exeins=mysqli_query($con,$ins);           
        ...
        //  }
           ...
           { 
            ...
            //   print_r($upsbal."h");die();
              ...
              $ins="INSERT INTO `sar_patti_payment`( `payment_id`, `payment_date`, `amount`, `payment_mode`, `balance`, `is_revoked`, `discount`, `discount_type`, `supplier_id`,`patti_id`) VALUES('$pay_id','$pay_date',$amt,'$payment_mode',$bal4,$is_revoked,$dis,'$amttype','$row[supplier_id]','$row[patti_id]')";           
    $exeins=mysqli_query($con,$ins);           
...
if($customerid!=""){   
  ...
  $ins="INSERT INTO `sar_sales_payment`( `payment_id`, `payment_date`, `amount`, `payment_mode`, `balance`, `is_revoked`, `discount`, `discount_type`, `customer_id`,`sales_no`) VALUES('$pay_id','$pay_date',$amt,'$payment_mode',$ba,$is_revoked,$dis,'$amttype','$row[customer_id]','$row[sales_no]')";           
  $exeins=mysqli_query($con,$ins);           
  ...
  $ins="INSERT INTO `sar_sales_payment`( `payment_id`, `payment_date`, `amount`, `payment_mode`, `balance`, `is_revoked`, `discount`, `discount_type`, `customer_id`,`sales_no`) VALUES('$pay_id','$pay_date',$amt,'$payment_mode',$bal2,$is_revoked,$dis,'$amttype','$row[customer_id]','$row[sales_no]')";           
  $exeins=mysqli_query($con,$ins);           
  ...
  $ins="INSERT INTO `sar_sales_payment`( `payment_id`, `payment_date`, `amount`, `payment_mode`, `balance`, `is_revoked`, `discount`, `discount_type`, `customer_id`,`sales_no`) VALUES('$pay_id','$pay_date',$amt,'$payment_mode',$bal3,$is_revoked,$dis,'$amttype','$row[customer_id]','$row[sales_no]')";           
  $exeins=mysqli_query($con,$ins);           
  ...
  $ins="INSERT INTO `sar_sales_payment`( `payment_id`, `payment_date`, `amount`, `payment_mode`, `balance`, `is_revoked`, `discount`, `discount_type`, `customer_id`,`sales_no`) VALUES('$pay_id','$pay_date',$amt,'$payment_mode',$val,$is_revoked,$dis,'$amttype','$row[customer_id]','$row[sales_no]')";           
  $exeins=mysqli_query($con,$ins);           
  ...
  $ins="INSERT INTO `sar_sales_payment`( `payment_id`, `payment_date`, `amount`, `payment_mode`, `balance`, `is_revoked`, `discount`, `discount_type`, `customer_id`,`sales_no`) VALUES('$pay_id','$pay_date',$amt,'$payment_mode',$bal4,$is_revoked,$dis,'$amttype','$row[customer_id]','$row[sales_no]')";           
  $exeins=mysqli_query($con,$ins);           
...
$sql="select * from sar_patti where supplier_id='$sid'";
$exe=mysqli_query($con,$sql);
$no=mysqli_num_rows($exe);
if($no==0){
