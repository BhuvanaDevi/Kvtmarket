...
$objPHPExcel->getActiveSheet()->setCellValue('P1','Inhand Trays');
$objPHPExcel->getActiveSheet()->setCellValue('Q1','Username');

$objPHPExcel->getActiveSheet()->setTitle("Get Settled Patti Report");
$data_sql=" SELECT * FROM `sar_patti` WHERE (patti_date>='".$from."' AND patti_date<='".$to."') AND payment_status=0 GROUP BY patti_id ";
...
// {
   ...
   // while($approval_row = mysqli_fetch_object($res)) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('P'.($i),$data[$j]['inhand_sum']);
        $objPHPExcel->getActiveSheet()->setCellValue('Q'.($i),$data[$j]['updated_by']);
...
$filename="Get Settled Patti Report";
