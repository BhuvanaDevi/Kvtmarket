...
// if($customer==""){
$sel_qryn = "SELECT * FROM `sar_opening_balance` WHERE (date >='$from' AND date<='$to')";
...
$pdf->Cell(180,10,'Opening Balance - Customer',0,0,'C');
$pdf->Ln(10);
$pdf->SetFont('Arial','B',12);
   ...
   while($result = $data_qryn->fetch(PDO::FETCH_ASSOC))
     {
      ...
      $sql = "SELECT * FROM  sar_customer WHERE customer_no='$result[name]'";
