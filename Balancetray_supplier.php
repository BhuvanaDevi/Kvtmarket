...
$data_qryn->execute();
   ...
   $sel_qryn = "SELECT * FROM `payment` WHERE supplierid='$customer' and (date>='$from' AND date<='$to') and pattid like 'P%'";
$data_qryn= $connect->prepare($sel_qryn);
$data_qryn->execute();
...
$pdf->Ln(15);
$pdf->SetFont('Arial','B',18);
$pdf->Cell(180,10,'Balance Tray -  Supplier',0,0,'C');
$pdf->Ln(10);
$pdf->SetFont('Arial','B',12);
...
$pdf->Ln(10);
      ...
      $pdf->Cell(20,10,'Bill',0,0,'R',true);
...
$results=$query->fetch(PDO::FETCH_OBJ);
      ...
      $pdf->Cell(50,10,'Supplier Name',0,0,'R',true);
      $pdf->Cell(50,10,'Date',0,0,'R',true);
      // $pdf->Cell(30,10,'Bill Amonut',0,0,'R',true);
      $pdf->Cell(50,10,'Total Bill Amount',0,0,'R',true);
    ...
    while($result = $data_qryn->fetch(PDO::FETCH_ASSOC))
     {
      ...
      //   $customer_id=$result['supplier'];
$saleid=$result['sale_id'];

        $sqlbal="select *,SUM(total_bill_amount) as tot from sar_patti where supplier_id='$supplier'";
      ...
      if($customer==""){ 
       ...
       $pdf->Cell(20,10,$result['inhand'],0,0,'R');
        ...
        $bal=$result['total_bill_amount']-$result['amount'];
         ...
         $pdf->Cell(50,10,$result['inhand'],0,0,'R');
      }
$pdf->Ln();
     }
// $pdf->SetFont('Arial','B',14);
