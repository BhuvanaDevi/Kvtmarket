...
// if($customer==""){
$sel_qryn = "SELECT * FROM `sar_ob_supplier` WHERE (date >='$from' AND date<='$to')";
...
$pdf->Cell(180,10,'Opening Balance - Supplier',0,0,'C');
$pdf->Ln(10);
...
$pdf->Ln(10);
   ...
   $pdf->SetFillColor(40,48,77);
      ...
      $pdf->Cell(50,10,'Amount',0,0,'R',true);
         $pdf->Ln(10);
         $pdf->SetFillColor(40,48,77);
               $pdf->SetTextColor(255,255,255);
   ...
   while($result = $data_qryn->fetch(PDO::FETCH_ASSOC))
     {
        ...
        $sql = "SELECT * FROM  sar_supplier WHERE supplier_no='$result[supplier_name]'";
        $query = $connect -> prepare($sql);
        $query->execute();
        $results=$query->fetch(PDO::FETCH_OBJ);
        
        $pdf->Cell(30,10,$result['group_name'],0,0,'R');
         $pdf->Cell(50,10,$results->contact_person,0,0,'R');
       $pdf->Cell(50,10,$result['date'],0,0,'R');
       $pdf->Cell(50,10,$result['amount'],0,0,'R');
